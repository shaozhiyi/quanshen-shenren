...
##   charge_units 撞击行程 = 玩家一次冲刺的位移 × 这个数（3.6 米 × 4 ≈ 14.4 米）
##   charge_speed_mult 冲撞速度 = 玩家奔跑速度（步行 ×2）× 这个数（10 × 1.5 = 15 米/秒）
...
const DEFS := {
	...
	"truck": {
		...
		# ---- 它的技能「锁定冲撞」：原地冻结 2 秒（地面铺红色撞击路径）→ 直线撞 4 个冲刺距离 ----
		"charge_lock": 2.0,
		"charge_units": 4.0,            # 3.6 米 × 4 ≈ 14.4 米
		"charge_speed_mult": 1.5,       # 玩家奔跑 10 米/秒 × 1.5 = 15 米/秒
