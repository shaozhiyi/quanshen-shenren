...
一辆橙色自卸重卡停在草原上，车头永远对着你。它不飞天、不蓄力、不砸地、不射星点，
但会三件事：以 3.6 米/秒缓慢朝你驶来（你步行 5 米/秒，走得掉；它会在离你约 6.3 米处
停下，不会把车头插进你身体）、在你贴到车身 7.3 米内时持续蹭你"尾气"，以及一招
**「等待出现」**——原地停 2 秒（轮子停转、脚下亮起橙色预警圈），到点瞬移出现在你
**正前方 2 个冲刺距离**（3.6 × 2 = 7.2 米）处。

所以它的实战角色是**会抄近路的厚血靶子**：给你一整段安心输出、用来试武器强化等级和
弓箭蓄力收益的沙包，同时把"外部 3D 模型 → BOSS"这条管线接通。想风筝它不再是无解的
——你退到 7.2 米开外它就直接跳到你面前，但贴脸缠斗时它反而不会跳（出手条件就是
"玩家比落点更远"），尾气伤害也不会因为它停着等跳而中断（掉血与驶近已解耦）。
...
所以玩家脚本只认"有没有这个方法"，大地图与白空间行为不变。瞬移落点同样过一遍
`confine()`，不会出现卡车跳到隔离带另一侧的怪事。
