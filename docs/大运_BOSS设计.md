...
车体与光波各自一轮只结算一次，**而且它一压到人就当场刹住**（不把你钉在车头上犁出去
二十多米），撞完冷却 7 秒才再出手。
...
> ⚠ 但**大运不能只按中心线判**：它身上那块 `StaticBody3D`（给剑当靶、顺便挡玩家）
> 是 10.6 米长，玩家会被车头顶在离车身中心约 5.6 米处，而中心线判定只有 ±2.2 米宽 ——
> 结果车能从人身上碾过去 28 米而一次伤害都不算（曾经真实发生过）。所以
> `_touch_player()` 判的是**整个车身的扫过足迹**：侧向 ≤ 半车宽 + 余量，纵向覆盖
> [-(半车长 + 余量), 本帧位移 + 半车长 + 余量]；并且**撞到人就把 `_charge_left` 归零**
> 当场刹住，免得把人钉在车头上犁着跑。回归见 `charge_live_selftest.gd`（那份刻意
> **不关任何进程**，让真实碰撞参与，才能抓到 unit 版看不见的这类问题）。
