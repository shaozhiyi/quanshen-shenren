...
（轮子停转，地面沿撞击路径铺出一条红色预警带：车宽 × 14.4 米的长条，两侧红实线 +
一排排朝前的箭头），到点沿**锁死的方向直线猛冲 4 个冲刺距离**（3.6 × 4 ≈ 14.4 米），
速度是你奔跑的 1.5 倍（10 × 1.5 = 15 米/秒，整段约 0.96 秒），撞上一次 20 血，然后冷却 2.5 秒。
...
横向躲得开），或者干脆顺着带子方向跑（你 10 米/秒，它 15 米/秒但只有 0.96 秒、14.4 米，
带子尽头之外就是安全区）。贴着它缠斗反而只会持续吃尾气，撞不到第二下。
...
`DIFF_AURA_MULT`。撞击伤害固定 20（三档一样，难度只改血量与尾气）。掉落照旧额外走一遍
