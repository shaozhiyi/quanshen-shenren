...
速度是你奔跑的 3 倍（10 × 3 = 30 米/秒，整段约 0.96 秒）。撞上一次扣 **20 血**，并且
**一瞬间被顺着车行方向顶开 2 个冲刺距离**（≈7.2 米，0.2 秒推完）；冲到底再甩出一圈
橙色光波，从停下处扩到约 12 米，**扫到人再扣 10**。车体与光波各自一轮只结算一次，
撞完冷却 7 秒才再出手。
...
1. ~~**冲锋碾压**~~ ✅ 已实现为「锁定冲撞」：锁位 2 秒（`charge_lock`）→ 沿钉死的方向
   ...
   命中同时调 `player.knockback(_charge_dir, charge_kb_dist())` 把玩家顶开
   `charge_knockback` 个冲刺距离（0.2 秒内推完，推完立刻交还操作权），
